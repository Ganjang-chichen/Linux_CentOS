...
<img src="../images/02/219prompt.png">

### 2.20
### 명령행 완성기능

    명령 입력 시 글자 일부분만 입력하고 TAB 키를 누를 시 나머지 부분을 자동완성 시켜준다.
    여러개가 존재하면 TAB키를 한번 더 눌러 목록을 확인할 수 있다.
...
<img src="../images/02/220history1.png">

<img src="../images/02/220history2.png">

|활용|설명|
|---|---|
|!!|마지막에 사용한 명령을 실행한다.|
|!n|n번째 사용한 명령을 실행한다.|
|!-n|사용한 명령 목록을 역으로 세어서 n번째 명령을 실행한다.|
|!string|가장 최근에 사용한 명령 중에서 "string"으로 시작하는 명령을 찾아서 실행한다.|
|!?string?|가장 최근에 사용한 명령 중에서 "string"이 표함된 명령을 찾아 실행한다.|
|^str1^str2|마지막에 사용한 명령문의 "str1"을  str2로 대체한 후에 실행한다.|

<img src="../images/02/220Excl.png">

### 2.22
### alias

    별명이라는 뜻으로 어떠한 명령을 지정해놓으면 사용자가지정해 놓은 명령이 대신 실행되게 한다.

    $ alias [별명='명령어']
    $ unalias 별명

<img src="../images/02/222alias.png">

### 2.23
### 명령행 편집기능

    명령행에서 이동을 빠르게 하거나. 손쉽게 삭제할 수 있는 기능 등을 제공한다.

    주요 이동 명령
    ctrl b  : 커서를 왼쪽 한 칸 이동
    ctrl f  : 커서를 오른쪽 한 칸 이동
    Esc b   : 커서를 왼쪽 한 단어 이동
    Esc f   : 커서를 오른쪽 한 단어 이동
    ctrl a  : 맨 왼쪽으로 이동
    ctrl e  : 맨 오른쪽으로 이동
...
### 명령 대체 기능
    ...
    특정 명령의 결과를 다른 명령의 인자 값으로 사용하는 것 이다. 
    결과 값을 얻을 명령어를 $() 안에 감싸면 된다.
    ex) ls -l $(which passwd)

<img src="../images/02/224substitution.png">

### 2.25
### 그룹 명령 실행

    하나의 명령행에 여러 개의 명령어를 동시에 사용하는 방법이다.
    A ; B   : A 실행 다음 B 실행
    A || B  : A 실행 성공시 결과출력, A 실패 시 B실행 후 결과 출력
    A && B  : A 가 실행 성공해야 B가 실행된다.

### 2.26
### 표준 입출력 제어 기능

    라눅스에서는 표준 입력과 표준 출력이라는 데이터 흐름을 사용한다.
    입력은 특정 프로그램으로 들어오는 값, 출력은 특정 프로그램에서 나오는 값이다.

    표준 입력(stdin : Standard Input) : 입력을 담당하는 키보드를 뜻한다. 셸에서는 숫자값 0 으로 표기하여 사용한다.
    표준 출력(stdout : Standard Output) : 명령의 결과값이 보여지는 화면을 말한다. 셸에서는 숫자값 1 로 표기한다. 일반적으로 생략 가능하다.
    표준 에러(stderr : Standard error) : 에러 값이 보여지는 화면을 말한다. 셸에서는 숫자값 2로 표기한다.

### 2.27
### 리다이랙션

    리다이랙션을 어떤 프로세스의 입/출력을 표준 입출력이 아닌 다른 입출력으로 변경할 때 사용한다.
    >   : 프로세스의 출력을 표준 출력에서 다른 출력으로 변경한다.
    >>  : 프로세스의 출력을 지정한 출력(보통 파일)에 추가한다.
    <   : 프로세스의 입력을 표준 입력에서 다른 입력으로 변경한다.
    <<  : 기호 뒤에 지정한 문자열을 입력으로 받아 해당 문자열이 나오기 전까지를 표준입력을 삼아 해당 내용을 출력한다.

    ex
    $ cat > tmp.txt     -> 표준 입력인 키보드로 입력한 내용을 tmp.txt 파일에 저장한다.
    $ cat >> tmp.txt    -> 입력한 내용을 tmp.txt 파일의 뒷 부분에 덧붙인다.
    $ wc < tmp.txt      -> 파일의 내용을 wc 의 입력으로 사용한다.
    $ cat << end end    -> 라는 문자열이 나오기 전 까지 표준입력으로 삼아 출력한다.

### 2.28
### 파이프

    파이프는 프로세스의 통신을 위해 도입한 것으로,
    어떤 프로세스의 표준 출력이 다른 프로세스의 표준 입력으로 쓰이게 하는것을 말한다.

    $ 명령어1 | 명령어2 -> 명령어 1의 결과값이 명령어2의 입력으로 사용된다.

<img src="../images/02/228pipe.png">

    tee : 파이프 연결 출력을 두 갈래로 나눌 때 사용하는 명령이다.

    $ 명령어1 | tee 명령어2 | 명령어3 -> 명령어1의 출력을 명령어 2와 명령어3의 입력으로 사용한다.

<img src="../images/02/228tee.png">

### 2.29
### 작업 제어 기능

    작업을 백그라운드와 포그라운드에서 실행할 수 있으며, 이 작업을 서로 전환하여 실행할 수 있다.
    상세 설명은 차후 프로세스 관리에서 설명한다.
...
### 프롬프트 제어 기능
    ...
    프롬프트를 원하는 대로 변경 지정이 가능하다.
    환경 변수인 PS1 을 이용해 변경가능하다.
...
### 확장된 내부 명령어
    ...
    alias 는 일시적으로만 적용되고 다음 로그인 시 적용되지 않는다.

    셸에서 사용되는 각종 특수문자들

    ~   : 홈 디렉털
    .   : 현재 디랙터리, 또는 명령행 맨 앞에서 source 라는 의미로 셸스크립트 등을 실행시킬 때도 사용된다.
    ..  : 한 단계 위 디렉터리
    ''  : 작은 따옴표는 모든 문자나 특수문자들을 일반 문자로 취급한다.
    ""  : 큰 따옴표는 $, `, \ 를 제외한 모든 문자들을 일반 문자로 취급한다.
    ``  : 역 따옴표는 명령 대체 기능을 수행하는데, 명령의 결과를 대체해서 사용한다.
    #   : 주석을 나타내는 기호로 환경설정 파일에서 직접적인 영향없이 설명을 달 때 사용한다.

<img src="../images/02/232quotes.png">

    $   : Shell 변수 기호로 뒤에 오는 문자열을 변수로 취급한다.
    &   : 특정 명령을 후면 작업으로 실행할 때 사용한다.
    *   : 아무 것도 없는 경우를 포함한 모든 문자를 뜻한다.
    ?   : 보통 한 문자를 대체할 때 사용한다. 특정 명령에 대한 결과를 Return 변수로도 쓰이는데, 셸상에서 내린 명령의 에러유무를 파악가능하다.
    ()  : 부속 shell(subshell)을 뜻하는 기호로 하나의 셸 단위로 묶어준다.
    \   : 탈출 문자로 바로 다음에 오는 특수문자의 기능을 없앤다. alias가 설정된 명령어 앞에 사용하는 경우에 alias를 없애준다.

<img src="../images/02/232backslash.png">

    []  : bracket이라 부르며 [과] 사이에 선택할 수 있는 문자를 나열하여 ~ 중에 하나라고 표현된다.
...
### 셸 프로그래밍
    ...
    셸 프로그래밍 : 셸에서 사용되는 여러 명령어들을 모아 하나의 파일로 만드는 과정
    셸 프로그래밍을 통해 만들어진 파일 또는 프로그램을 셸 스크립트라 한다.
    셸은 for, case 같은 구문, 변수 함수 등과 같이 보통 언어가 가지고 있는 기본적인 특성을 가지고있다.
    ...
    두 번째 줄 부터 원하는 명령 및 구문 등을 입력한다.

    echo "I am a star~ " 

    스크립트 작성을 완료했으면, 권한 변경을 통해 실행가능파일로 바꾼 뒤, 실행한다.
    이 때 ./ 을 붙여야된다.
    $ ./temp.sh
    또는 실행파일로 바꾸지 않더라도 다음과 같은 방법으로 쓸 수 있다.
    $ . temp.sh
    $ sh temp.sh
    $ source temp.sh


<img src="../images/02/233shellscript.png">

    - $IFS 
    입력 필드 구분자로서 셸 상에서 입력을 읽어 들일 때 글자를 구분하기 위한 목적으로 사용되는 문자 목록이다.
    일반적으로 스페이스, 탭, 개행문자를 사용한다.
    ...
    $# : 스크립트에 넘겨진 아규먼트의 수
    $* : 스크립트에 넘겨진 인자 전체를 하나의 변수에 저장하면 IFS 변수의 첫 번째 문자로 구분
    $@ : $*와 동일하나 차이점은 IFS 변수를 사용하지 않음
    $? : 실행한 뒤의 반환 값, 즉 참이면 0, 거짓이면 1이 반환됨
    $- : 현재 Shell이 호출될 대 사용한 옵션들

[예제](./script/233argument.sh)

    - set
    일반적으로 셸 변수를 출력하는 명령이다.
    리눅스에서는 셸의 환경과 관련된 변수 함수 등오 출력한다.

    - env
    환경 변수를 출력하는 명령이다.
    export 된 변수는 이 명령으로 확인해야하고, 사용자가 선언한 셸 변수 정보도 함께 출력한다.
